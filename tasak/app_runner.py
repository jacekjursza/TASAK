...
def _run_proxy_mode(meta: Dict[str, Any], app_args: List[str]):
    ...
    if not base_command:
        print(f"'command' not specified in app configuration for proxy mode.", file=sys.stderr)
        sys.exit(1)
...
def _run_curated_mode(meta: Dict[str, Any], app_args: List[str]):
    ...
    if not base_command:
        print(f"'command' not specified for curated mode.", file=sys.stderr)
        sys.exit(1)

    parser = argparse.ArgumentParser(description=meta.get('description', 'Curated command'))
